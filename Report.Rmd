...
# Create a plot that shows the number of unique SNP locations as a percentage of gene length
total_length <- max(gff$end)
gene_per <- gene_table %>%
  mutate(length = end - start) %>%
  mutate(percentage = length / total_length) %>%
  select(gene_name, length, percentage) %>%
  knitr::kable(col.names = c("gene",
                             "length",
                             "percentage"))

# Add a column in unique SNPs data frame that shows gene lengths and percentage
unique_snps_per <- cbind
