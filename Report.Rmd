...
**Table 2**: Mutation positions and the genes they are found within [@pachetti2020emerging][@korber2020][@yang2020]. ***Note:*** Positions 3036, 17746, and 26143, which were identified by Pachetti et al., were not identified in the analyses by van Dorp et al. Instead, adjacent position 3037, 17747, and 26144 were identified, with a relatively large number of isolates having the mutation with high quality reads, which have been included here.
...
isolation_source_plot <- ggplot(isolation_source, aes(x = Isolation_source,
                              y = n)) +
...
exp_v_obs_plot <- ggplot(unique_snps_per, aes(fill = expect_v_actual,
                            x = gene,
                            y = snp_values)) +
...
key_mutations_plot <- key_mutations_all %>%
...
```
**Figure 3**: Number of samples from this specific Beijing data set with common mutations found in samples globally. ***Note:*** Positions 3036, 17746, and 26143, which were identified by Pachetti et al., were not identified in the analyses by van Dorp et al. Instead, adjacent position 3037, 17747, and 26144 were identified, with a relatively large number of isolates having the mutation with high quality reads, which have been included here.
...
beijing_total_confirmed_plot <- beijing_epi_tab %>%
  ...
  labs(x = "Date",
       ...
       title = 
         "Total Number of Confirmed Cases of COVID-19 in Beijing Over Time") +
...
china_daily_cases_plot <- ggplot(chn_new_cases, 
...
china_stringency_plot <- chn_ourworld_tab %>%
  ggplot(aes(x = date,
             y = stringency_index)) +
  geom_line(color = "#00BFC4") +
  labs(x = "Date",
